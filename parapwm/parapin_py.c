...
//  newsig.sa_handler = sigint_handler;
//  newsig.sa_mask = SIGINT;
//  newsig.sa_flags = 0;
//  sigaction(SIGINT, &newsig, &oldsig);
  ...
  sleepon.tv_nsec = (msecson % 1000) * 1000000 + nsecson;
  sleepoff.tv_sec = msecsoff / 1000;
  sleepoff.tv_nsec = (msecsoff % 1000) * 1000000 + nsecsoff;

  while (n > 0) {
    change_pin(self->pin, 1);
    nanosleep(&sleepon, NULL);
    n --;
    if (n <= 0)
      break;
    change_pin(self->pin, 0);
    nanosleep(&sleepoff, NULL);
...
initparapwm_c(void) 
{
  ...
  module = Py_InitModule3("parapwm_c", module_methods,
			  "parapin module.");
  
  constants = Py_InitModule3("parapwm_c.CONST", NULL, "Parapin constants");
  ...
  parapin_error = PyErr_NewException("parapwm_c.ParapinError", NULL, NULL);
